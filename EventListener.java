...
public class EventListener implements Listener {
	...
	public void OnRespawn(PlayerRespawnEvent event) {
		...
		if (Main.getMe().deadParticipants.contains(player)) {
			...
			event.setRespawnLocation(Main.getMe().originalLocations.get(player.getName()));
			Main.getMe().originalLocations.remove(player.getName());

			event.getPlayer().sendMessage(
					Main.getMe().sbf + ChatColor.RED + "You lost all of your lives! Good luck next game!");
	...
	public void OnDeath(PlayerDeathEvent event) {
		...
		if (Main.getMe().isPlayerInArena(player)) {
			Main.getMe().deadParticipants.add(player);
	...
	public void OnSnowballHit(EntityDamageByEntityEvent event) {
		...
		if (!(event.getDamager() instanceof Snowball) && Main.getMe().isPlayerInArena(hit)) {
			event.setCancelled(true);
		}
	}
